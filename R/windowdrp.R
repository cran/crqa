...
windowdrp <- function(ts1, ts2, step, windowsize, lagwidth, datatype, 
                      radius = 0.001,  delay = 1, embed = 1, rescale, 
                      normalize = 0, mindiagline = 2, minvertline = 2,
                      tw = 0){
  
ts1 = as.vector(as.matrix(ts1));   ts2 = as.vector(as.matrix(ts2));
dpoint = seq(1, (length(ts1) - (windowsize)-1), step)
...
for (i in dpoint){
  ...
  drpd = c(drpd,
      mean(drpdfromts(ts1win, ts2win, lagwidth, datatype, radius, 
                      delay, embed, rescale, normalize, mindiagline, 
                      minvertline, tw)$profile))
